...
The installation documentation was also smoke-tested with `skills@1.5.25` and Node 22.20.0 in a fresh OS temporary project, using this checkout as the local source and selecting `codex claude-code`, all nine skills, and project scope. The runtime was supplied through npm's temporary package execution; the system Node version was not upgraded. Telemetry was disabled. Installation succeeded; all 32 source files, including references and license notices, matched the installed files byte-for-byte. All nine Claude links resolved to their Codex-compatible `.agents/skills` copies. The installed directories were snapshots, not links back to the source checkout. No matching global skills were present after the test. GitHub installation awaits publication; global installation/removal, native client invocation, and update execution were not tested.

Removal was tested in that same isolated project after adding a synthetic unrelated skill and a Claude link to it. Removing the nine names with `-a codex claude-code` reported success and removed Claude's links, but a filesystem assertion failed because the shared `.agents/skills` copies remained. The CLI retains shared directories used by other detected agents. Repeating named removal without the agent filter removed all nine copies and links; filesystem assertions confirmed that the unrelated skill, its link, and all source checkout entrypoints survived. The docs therefore distinguish complete removal across agents from host-scoped removal that may retain shared copies. Test commands used `-y` only inside the temporary project; published examples omit it, though the CLI also skips confirmation when it detects an agent.
